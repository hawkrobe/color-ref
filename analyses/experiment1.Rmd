...
deltae <- colors %>%
  ...
  mutate(deltae = spacesXYZ::DeltaE(matrix(c(L.x, a.x, b.x), ncol = 3), 
                                        matrix(c(L.y, a.y, b.y), ncol = 3), 
                                        metric = '2000')) %>%
  group_by(word) %>%
  summarize(deltae = mean(deltae))
```
...
Some evidence for concreteness as non-redundant with imagability
...
But only when predicting entropy, not delta-e

```{r}
linearMod <- lm(deltae ~ imageability + concreteness, data=dCompiled)
summary(linearMod)
