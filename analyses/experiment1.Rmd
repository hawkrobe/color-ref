...
# Reproduced analysis from Python scripts
...
```
## Compiled variance metrics and Glasgow ratings from python analyses and R reproduction
```{r}
...
### Compare regression models for predicting entropy 

No evidence for concreteness as non-redundant with imageability -- no evidence for random effects

```{r}
entropyMod.img <- dCompiled %>%
  lm(entropy ~ imageability,
     data = .)

entropyMod.both <- dCompiled %>%
  lm(entropy ~ imageability + concreteness,

anova(linearModImg, linearModBoth, test="Chisq")

entropyMod.lmer <- dCompiled %>%
  lmer(entropy ~ imageability + concreteness + (1 | wordSetID), 
       REML = F,
       data=.)

anova(entropyMod.lmer, entropyMod.both, entropyMod.img)
```

This is the best-fitting model for entropy.

```{r}
summary(entropyMod.img)
```

Concreteness has small non-redundent effect when predicting delta-E. 

```{r}
deltaeMod.img <- dCompiled %>%
  lm(deltae.pop ~ imageability,
     data = .)

deltaeMod.both <- dCompiled %>% 
  lm(deltae.pop ~ imageability + concreteness, 
     data = .)

deltaeMod.lmer <- dCompiled %>%
  lmer(deltae.pop ~ imageability + concreteness + (1 | wordSetID), 
       data = .)

anova(deltaeMod.lmer, deltaeMod.both, deltaeMod.img)
```

This is the best-fitting model for delta-e
...
Why? One observation is that these words tend to have high imageability but low concreteness and are at the low end of the entropy/delta-e scales.
These scales pull apart because of the slight warping of the space of munsell chips (e.g. with more blue chips than yellow chips, meaning lower delta-e than entropy) so these additional data points must interact with that different between measures.
In any case, the predictive power of imageability appears stronger in all cases than any marginal effect of concreteness.
...
linearMod <- lmer(deltae.pop ~ imageability + concreteness + (1|wordSetID), data=dCompiled)
summary(linearMod)

linearMod <- lmer(entropy ~ imageability + concreteness + (1|wordSetID), data=dCompiled)
summary(linearMod)
```
...
dIntraparticipant %>%
  ...
  left_join(deltae.pop) %>%
  ggplot(aes(x = deltae.internal, y = deltae.pop)) +
  geom_point() + 
  geom_abline(slope = 1, intercept = 0, linetype = 'dotted') +
  geom_errorbarh(aes(xmin = ci_lower, xmax = ci_upper), width = 0, alpha = 0.25) +
  geom_smooth(method = 'lm', formula = y ~ poly(x,1), color = 'red') +
  labs(y="Average delta E between different participants", 
       x= "Average delta E within participant") +
  xlim(0, 43) +
  ylim(0, 43) +
  ggthemes::theme_few() +
...
linearMod <- lm(log(empirical_stat_groundTruth) ~ empirical_stat_sliderResponse, data=tmp)
summary(linearMod)
...
compare <- deltaE_block1[order(deltaE_block1$word),] %>%
  left_join(deltae.pop) %>%
  rename(bothBlocks = deltae.pop, block1 = deltaE)
