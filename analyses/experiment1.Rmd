...
```{r}
d.raw = read_csv('../data/norming/color.csv', show_col_types=F) 
```
...
expectations <- read_csv('../data/norming/expectations.csv') %>%
  ...
  # merge in participant's own response on that trial
  left_join(d %>% 
              filter(condition == 'block2_target_trial') %>% 
              select(word, button_pressed, participantID, condition)) %>%
  select(-condition) %>%
    
  # merge in other participants' responses (on both both blocks)
...
expectations.boot <- expectations %>%
  filter(participantID.x %in% d$participantID) %>%
  # Collapse to match value for each participant-word pair
  ...
  mutate(expected = scale(sliderResponse),
         actual = log(match / (1-match)),
         actual_alt = log(match)) %>%
  gather(metric, value, expected, actual, actual_alt) %>%
  group_by(word, metric) %>%
  # bootstrap error bars
  tidyboot::tidyboot_mean(value, nboot = 100, na.rm = T) %>%
  select(-mean, -n) %>%
...
```{r}
expectations.boot %>%
  ggplot(aes(x = empirical_stat_expected, y = empirical_stat_actual_alt)) +
  geom_point() +
  geom_errorbarh(aes(xmax = ci_upper_expected, xmin = ci_lower_expected), alpha = 0.25) +
  geom_errorbar(aes(ymax = ci_upper_actual_alt, ymin = ci_lower_actual_alt), alpha = 0.25) +
  geom_smooth(method = 'lm', formula = y ~ poly(x, 1), color = 'red') +
...
```{r}
tmp %>%
  ggplot(aes(x = empirical_stat_expected, y = empirical_stat_actual)) +
  geom_point() +
  geom_errorbarh(aes(xmax = ci_upper_expected, xmin = ci_lower_expected), alpha = 0.25) +
  geom_errorbar(aes(ymax = ci_upper_actual, ymin = ci_lower_actual), alpha = 0.25) +
  geom_smooth(method = 'lm', formula = y ~ poly(x, 1), color = 'red') +
  labs(x = 'expectation', y = 'true log odds (p / 1 - p)') +
  ggthemes::theme_few() +
  theme(aspect.ratio = 1)
