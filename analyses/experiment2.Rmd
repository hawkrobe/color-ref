...
```{r}
conditions.toplot <- clicks %>%
  group_by(blockNum, condition) %>%
  tidyboot::tidyboot_mean(correct) %>%
  mutate(dist_quantile = ifelse(condition == 'concrete', 2, 4))

conditions.toplot %>% filter(blockNum %in% c(0,5))
```

```{r}
quantiles.toplot <- clicks %>%
...
clicks %>%
  ggplot(aes(x = blockNum + 1, y = empirical_stat, color = as.numeric(dist_quantile))) +
    geom_line(aes(group = as.numeric(dist_quantile)),
              stat="smooth",method = "lm", se = F, alpha = 0.75, linetype='dashed',
              data = quantiles.toplot) +
    geom_line(aes(group = condition), size = 1, 
              data = conditions.toplot) +
    geom_text(aes(label=condition),  nudge_x = -0.4, nudge_y = -.2, vjust = "inward",
              data = conditions.toplot %>% filter(blockNum == 5)) +
    geom_point(aes(group = condition), size = 2, 
              data = conditions.toplot) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper, group = condition), width = 0,
              data = conditions.toplot) +
